...
public class Ex02For {
    public static void main(String[] args) {
        ...
        //2. 1부터 사용자가 새로 입력한 숫자까지의 합을 수하는
        //  for무을 작성 하세요
        
        // 이문제 같은 경우에는 합을 보관할 변수가 밖에 선언과 초기화 되어야
        // for문이 반복되더라고 그값이 남아있게 된다.
        int sum = 0;
        System.out.println("합을 구할 숫자: ");
        number = scanner.nextInt();
        for(int i = 1; i <=number; i++) {
                sum+= i;
        }
        
        System.out.printf("1부터 %d까지의 합: %d/n", number, sum);
        ...
        System.out.printf("1부터 %d까지의 곱: %d|n",  number, sigma);
    }else {
